...
//
// NOTE: This is largely copied from k8s.io/kubernetes/pkg/api/validation/schema.go, with some
// slight modifications, such as rebasing onto the newer versions of client-go/apimachinery object
// types.
...
func (i *InvalidTypeError) Error() string {
	return fmt.Sprintf(
		"expected type %s, for field %s, got %s", i.ExpectedKind.String(), i.FieldName, i.ObservedKind.String())
}
...
// NewSwaggerSchemaFor returns the SwaggerSchema object ready to validate objects of given
// GroupVersion
func NewSwaggerSchemaFor(
	delegate discovery.SwaggerSchemaInterface, gv schema.GroupVersion,
) (*SwaggerSchema, error) {
	glog.V(3).Infof("Fetching schema for %v", gv)
...
// validateList unpacks a list and validate every item in the list. It return nil if every item is
// ok. Otherwise it return an error list contain errors of every item.
...
func (s *SwaggerSchema) ValidateObject(obj interface{}, fieldName, typeName string) []error {
	...
	// Verify the api version matches.  This is required for nested types with differing api versions
	// because s.api only has schema for 1 api version (the parent object type's version). e.g. an
	// extensions/v1beta1 Template embedding a /v1 Service requires the schema for the
	// extensions/v1beta1 api to delegate to the schema for the /v1 api. Only do this for !ok objects
	// so that cross ApiVersion vendored types take precedence.
	...
	for key, value := range fields {
		...
		if value == nil {
			glog.V(3).Infof("Skipping nil field: %s%s", fieldName, key)
			continue
		}
...
// delegateIfDifferentAPIVersion delegates the validation of an object if its ApiGroup does not
// match the current SwaggerSchema.
//
// First return value is true if the validation was delegated (by a different ApiGroup
// SwaggerSchema) Second return value is the result of the delegated validation if performed.
func (s *SwaggerSchema) delegateIfDifferentAPIVersion(
...
// isDifferentAPIVersion Returns true if obj lives in a different ApiVersion than the SwaggerSchema
// does. The SwaggerSchema will not be able to process objects in different ApiVersions unless they
// are vendored.
...
func (s *SwaggerSchema) validateField(
	...
	// TODO: caesarxuchao: because we have multiple group/versions and objects may reference objects
	// in other group, the commented out way of checking if a filedType is a type defined by us is
	// outdated. We use a hacky way for now.
	//
	// TODO: the type name in the swagger spec is something like "v1.Binding", and the "v1" is
	// generated from the package name, not the groupVersion of the type. We need to fix go-restful to
	// embed the group name in the type name, otherwise we couldn't handle identically named types in
	// different groups correctly.
	...
	// API servers before release 1.3 produce swagger spec with `type: "any"` as the fallback type,
	// while newer servers produce spec with `type: "object"`. We have both here so that kubectl can
	// work with both old and new api servers.
	case "object":
	case "any":
	default:
