...
// Retry helpers.
//
// A collection of helpers methods making it easy to poll the Kubernetes API until some operation on
// a Kubernetes resource is complete. Builds on top of `StateChangeConf`.
//
// NOTE: influenced very heavily by:
// github.com/hashicorp/terraform/blob/v0.11.7/helper/resource/wait.go
...
func Retry(timeout time.Duration, f RetryFunc) error {
	...
	c := &StateChangeConf{
		...
		Refresh: func() (*unstructured.Unstructured, string, error) {
			...
			if rerr == nil {
				resultErr = nil
				return &unstructured.Unstructured{}, "success", nil
			}

			resultErr = rerr.Err

			if rerr.Retryable {
				return &unstructured.Unstructured{}, "retryableerror", nil
			}
