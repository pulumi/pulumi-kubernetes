...
// Kubernetes API object state polling.
//
// Allows us to check the API server for state changes. Primarily useful for checking whether
// Kubernetes has initialized some resource.
//
// NOTE: This is very highly influenced by
// github.com/hashicorp/terraform/blob/v0.11.7/helper/resource/state.go.
...
type StateRefreshFunc func() (result *unstructured.Unstructured, state string, err error)
...
func (conf *StateChangeConf) WaitForState() (*unstructured.Unstructured, error) {
	glog.V(7).Infof("Waiting for state to become: %s", conf.Target)
	...
	go func() {
		...
		for {
			...
			glog.V(5).Infof("Waiting %s before next try", wait)
		}
	}()
	...
	for {
		...
		case <-timeout:
			glog.V(3).Infof("WaitForState timeout after %s", conf.Timeout)
			glog.V(3).Infof("WaitForState starting %s refresh grace period", refreshGracePeriod)
		...
		forSelect:
			for {
				...
				case r, ok := <-resCh:
					...
					lastResult = r
				case <-timeout:
					glog.V(2).Infof("WaitForState exceeded refresh grace period")
					break forSelect
				}
			}
