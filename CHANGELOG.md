...
- The new `enableSecretMutable` provider configuration option treats changes to
  ...
  by setting its `immutable` field to `true`, or by setting its
  `replaceOnChanges` resource option to `[".stringData", ".data"]`.
  ...
  **Note:** These options (`enableSecretMutable` and `enableConfigMapMutable`)
  ...
  (usually to trigger updates for downstream dependencies like `Deployments`)
