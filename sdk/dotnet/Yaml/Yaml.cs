...
namespace Pulumi.Kubernetes.Yaml
{
    ...
    public abstract class CollectionComponentResource : ComponentResource
    {
        ...
        protected static CustomResourceOptions GetChildOptions(Pu.Resource parent, InputList<Pu.Resource>? extraDependsOn, ComponentResourceOptions? options)
        {
        ...
        protected internal static InvokeOptions GetInvokeOptions(CustomResourceOptions? options)
        {
    ...
    {
        public static Output<ImmutableDictionary<string, KubernetesResource>> Parse(ConfigGroupArgs config, bool glob, CustomResourceOptions? opts)
        {
            ...
            var yamls = new InputList<string>();
            if (config.Yaml != null)
                yamls.AddRange(config.Yaml);
            ...
            if (config.Files != null)
            {
                foreach (var file in config.Files)
                {
                    ...
                    if (IsUrl(file))
                    {
                        ...
                        var yaml = wc.DownloadString(file);
                        yamls.Add(yaml);
                    } 
                    else 
                    {
                        ...
                        {
                            var yaml = File.ReadAllText(fileId);
                            yamls.Add(yaml);
                        }
                    }
                }
            ...
            if (yamls is not null)
            {
                var invokeOpts = CollectionComponentResource.GetInvokeOptions(opts);
                var yamlResources = yamls.ToOutput().Apply(texts =>
                {
                    var r = texts
                        .Select(text =>
                            ParseYamlObjects(new ParseArgs
                            {
            ...
            if (config.Objs != null)
            {
                var docResources = ParseYamlObjects(new ParseArgs
                {
                    Objs = config.Objs,
                    Transformations = transformations,
                    ResourcePrefix = config.ResourcePrefix
        ...
        internal static Output<ImmutableDictionary<string, KubernetesResource>> ParseYamlObjects(ParseArgs config,
            CustomResourceOptions? opts)
        {
            return config.Objs.ToOutput().Apply(objs =>
            {
                ...
                var inputs = objs
                    .SelectMany(obj => ParseYamlObject(obj, config.Transformations, config.ResourcePrefix, opts))
        ...
        private static Output<(string, KubernetesResource)>[] ParseYamlObject(ImmutableDictionary<string, object> obj,
            List<TransformationAction>? transformations, string? resourcePrefix, CustomResourceOptions? opts)
        {
            ...
            opts = CustomResourceOptions.Merge(null, opts);
