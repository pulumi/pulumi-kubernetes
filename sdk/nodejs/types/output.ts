 ...
 */
   ...
   * change the apiVersion and would not touch any other field in the CR. - `Webhook`: API Server
   * will call to an external webhook to do the conversion. Additional information is needed for
   * this option.
   */
 ...
 */
   ...
   * Defaults to a created-at column. Optional, the global columns for all versions. Top-level and
   ...
   */
      ...
      /**
   * Subresources describes the subresources for CustomResource Optional, the global subresources
 ...
 */
   ...
   * Per-version columns must not all be set to identical values (top-level columns should be used
   * instead) This field is alpha-level and is only honored by servers that enable the
   ...
   * Top-level and per-version schemas are mutually exclusive. Per-version schemas must not all be
   * set to identical values (top-level validation schema should be used instead) This field is
   * alpha-level and is only honored by servers that enable the CustomResourceWebhookConversion
   * feature.
   */
   ...
   * subresources are mutually exclusive. Per-version subresources must not all be set to identical
   * values (top-level subresources should be used instead) This field is alpha-level and is only
 ...
 */
   ...
   */
    ...
    /**
 * WebhookClientConfig contains the information to make a TLS connection with the webhook. It has
 * the same field as admissionregistration.v1beta1.WebhookClientConfig.
 */
    export interface WebhookClientConfig {
      /**
   * `caBundle` is a PEM encoded CA bundle which will be used to validate the webhook's server
 ...
 */
   ...
   * copy of this pod on every node that matches the template's node selector (or on every node if
 ...
 */
   ...
   * number of pods.  Ergo, metrics used must decrease as the pod count is increased, and
 ...
 */
   ...
   * parallelism to have any positive value.  Setting to 1 means that parallelism is limited to 1
   ...
   * `manualSelector=true` in jobs that were created with the old `extensions/v1beta1` API. More
   * info:
 ...
 */
   ...
   */
      ...
      /**
   * Expected values Shared: multiple blob disks per storage account  Dedicated: single blob disk
   * per storage account  Managed: azure managed data disk (only in managed availability set).
   * defaults to shared
   */
      readonly kind: string
 ...
 */
   ...
   * '_' or '.'. Values with non-UTF-8 byte sequences must use the BinaryData field. The keys stored
   * in Data must not overlap with the keys in the BinaryData field, this is enforced during
   * validation process.
   */
 ...
 */
    export interface EmptyDirVolumeSource {
      /**
   * What type of storage medium should back this directory. The default is "" which means to use
   * the node's default medium. Must be an empty string (default) or Memory. More info:
 ...
 */
   ...
   * to a node, and all of the containers have been created. At least one container is still
   * running, or is in the process of starting or restarting. Succeeded: All containers in the pod
   * have terminated in success, and will not be restarted. Failed: All containers in the pod have
 ...
 */
   ...
   * error unless it is marked optional. Paths must be relative and may not contain the '..' path or
   * start with '..'.
   */
      readonly items: core.v1.KeyToPath[]
 ...
 */
   ...
   * specified for headless services when proxying is not required. Only applies to types ClusterIP,
   ...
   * or cluster-wide endpoints. "Local" preserves the client source IP and avoids a second hop for
 ...
 */
   ...
   * default behaviour. Set to "default" if you are not using namespaces within StorageOS.
 ...
 */
   ...
   * contain characters disallowed from the conventional "path" part of a URL as defined by RFC
   * 3986. Paths must begin with a '/'. If unspecified, the path defaults to a catch all sending
 ...
 */
    export interface NetworkPolicyPort {
      /**
   * If specified, the port on the given protocol.  This can either be a numerical or named port on
   * a pod.  If this field is not provided, this matches all port names and numbers. If present,
 ...
 */
   ...
   * are no NetworkPolicies selecting the pod OR if the traffic source is the pod's local node, OR
   * if the traffic matches at least one ingress rule across all of the NetworkPolicy objects whose
   ...
   * you must specify a policyTypes value that include "Egress" (since such a policy would not
 ...
 */
   ...
   * of allowed sysctls. Single * means all unsafe sysctls are allowed. Kubelet has to whitelist all
   ...
   * the PSP should deny the pod. If set to false the container may run with a read only root file
 ...
 */
   ...
   * the background; 'Foreground' - a cascading policy that deletes all dependents in the
   * foreground.
   */
...
export namespace networking {
  export namespace v1 {
    /**
 * IPBlock describes a particular CIDR (Ex. "192.168.1.1/24") that is allowed to the pods matched by
 * a NetworkPolicySpec's podSelector. The except entry describes CIDRs that should not be included
 * within this rule.
 */
    export interface IPBlock {
      /**
 ...
 */
   ...
   * rule across all of the NetworkPolicy objects whose podSelector matches the pod. If this field
   * is empty then this NetworkPolicy does not allow any traffic (and serves solely to ensure that
 ...
 * external MyAPIObject. That causes the raw JSON to be stored, but not unpacked. The next step is
 * to copy (using pkg/conversion) into the internal struct. The runtime package's DefaultScheme has
 ...
 */
   ...
   */
      ...
      /**
   * NonResourceURLs is a set of partial urls that a user should have access to.  *s are allowed,
   * but only as the full, final step in the path Since non-resource URLs are not namespaced, this
   * field is only applicable for ClusterRoles referenced from a ClusterRoleBinding. Rules can
 ...
 */
   ...
   */
      ...
      /**
   * NonResourceURLs is a set of partial urls that a user should have access to.  *s are allowed,
   * but only as the full, final step in the path This name is intentionally different than the
   * internal type so that the DefaultConvert works nicely and because the ordering may be
   * different. Since non-resource URLs are not namespaced, this field is only applicable for
