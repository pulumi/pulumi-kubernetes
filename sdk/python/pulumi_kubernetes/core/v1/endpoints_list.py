...
class EndpointsList(pulumi.CustomResource):
    ...
    List of endpoints.
      ...
      * `subsets` (`list`) - The set of all endpoints is the union of all subsets. Addresses are placed into subsets according to the IPs they share. A single address with multiple ports, some of which are ready and some of which are not (because they come from different containers) will result in the address being displayed in different subsets for the different ports. No address will appear in both Addresses and NotReadyAddresses in the same subset. Sets of addresses and ports that comprise a service.
        * `addresses` (`list`) - IP addresses which offer the related ports that are marked as ready. These endpoints should be considered safe for load balancers and clients to utilize.
          ...
          * `node_name` (`str`) - Optional: Node hosting this endpoint. This can be used to determine endpoints local to a node.
          * `targetRef` (`dict`) - Reference to object providing the endpoint.
            * `api_version` (`str`) - API version of the referent.
        ...
        * `notReadyAddresses` (`list`) - IP addresses which offer the related ports but are not currently marked as ready because they have not yet finished starting, have recently failed a readiness check, or have recently failed a liveness check.
        * `ports` (`list`) - Port numbers available on the related IP addresses.
          ...
          * `port` (`float`) - The port number of the endpoint.
    ...
    def __init__(__self__, resource_name, opts=None, api_version=None, items=None, kind=None, metadata=None, __props__=None, __name__=None, __opts__=None):
        ...
        :param pulumi.Input[list] items: List of endpoints.
        ...
        The **items** object supports the following:
          ...
          * `subsets` (`pulumi.Input[list]`) - The set of all endpoints is the union of all subsets. Addresses are placed into subsets according to the IPs they share. A single address with multiple ports, some of which are ready and some of which are not (because they come from different containers) will result in the address being displayed in different subsets for the different ports. No address will appear in both Addresses and NotReadyAddresses in the same subset. Sets of addresses and ports that comprise a service.
            * `addresses` (`pulumi.Input[list]`) - IP addresses which offer the related ports that are marked as ready. These endpoints should be considered safe for load balancers and clients to utilize.
              * `hostname` (`pulumi.Input[str]`) - The Hostname of this endpoint
              * `ip` (`pulumi.Input[str]`) - The IP of this endpoint. May not be loopback (127.0.0.0/8), link-local (169.254.0.0/16), or link-local multicast ((224.0.0.0/24). IPv6 is also accepted but not fully supported on all platforms. Also, certain kubernetes components, like kube-proxy, are not IPv6 ready.
              * `node_name` (`pulumi.Input[str]`) - Optional: Node hosting this endpoint. This can be used to determine endpoints local to a node.
              * `targetRef` (`pulumi.Input[dict]`) - Reference to object providing the endpoint.
                * `api_version` (`pulumi.Input[str]`) - API version of the referent.
            ...
            * `notReadyAddresses` (`pulumi.Input[list]`) - IP addresses which offer the related ports but are not currently marked as ready because they have not yet finished starting, have recently failed a readiness check, or have recently failed a liveness check.
            * `ports` (`pulumi.Input[list]`) - Port numbers available on the related IP addresses.
              * `appProtocol` (`pulumi.Input[str]`) - The application protocol for this port. This field follows standard Kubernetes label syntax. Un-prefixed names are reserved for IANA standard service names (as per RFC-6335 and http://www.iana.org/assignments/service-names). Non-standard protocols should use prefixed names such as mycompany.com/my-custom-protocol. Field can be enabled with ServiceAppProtocol feature gate.
              * `name` (`pulumi.Input[str]`) - The name of this port.  This must match the 'name' field in the corresponding ServicePort. Must be a DNS_LABEL. Optional only if one port is defined.
              * `port` (`pulumi.Input[float]`) - The port number of the endpoint.
        ...
        super(EndpointsList, __self__).__init__(
    ...
    def get(resource_name, id, opts=None):
        ...
        return EndpointsList(resource_name, opts=opts, __props__=__props__)
