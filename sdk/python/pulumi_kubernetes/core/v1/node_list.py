...
class NodeList(pulumi.CustomResource):
    ...
    List of nodes
      ...
      * `spec` (`dict`) - Spec defines the behavior of a node. https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
      ...
      * `status` (`dict`) - Most recently observed status of the node. Populated by the system. Read-only. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
    ...
    def __init__(__self__, resource_name, opts=None, api_version=None, items=None, kind=None, metadata=None, __props__=None, __name__=None, __opts__=None):
        ...
        :param pulumi.Input[list] items: List of nodes
        ...
        The **items** object supports the following:
          ...
          * `spec` (`pulumi.Input[dict]`) - Spec defines the behavior of a node. https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
          ...
          * `status` (`pulumi.Input[dict]`) - Most recently observed status of the node. Populated by the system. Read-only. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
            * `addresses` (`pulumi.Input[list]`) - List of addresses reachable to the node. Queried from cloud provider, if available. More info: https://kubernetes.io/docs/concepts/nodes/node/#addresses Note: This field is declared as mergeable, but the merge key is not sufficiently unique, which can cause data corruption when it is merged. Callers should instead use a full-replacement patch. See http://pr.k8s.io/79391 for an example.
              * `address` (`pulumi.Input[str]`) - The node address.
              * `type` (`pulumi.Input[str]`) - Node address type, one of Hostname, ExternalIP or InternalIP.

            * `allocatable` (`pulumi.Input[dict]`) - Allocatable represents the resources of a node that are available for scheduling. Defaults to Capacity.
            * `capacity` (`pulumi.Input[dict]`) - Capacity represents the total resources of a node. More info: https://kubernetes.io/docs/concepts/storage/persistent-volumes#capacity
            * `conditions` (`pulumi.Input[list]`) - Conditions is an array of current observed node conditions. More info: https://kubernetes.io/docs/concepts/nodes/node/#condition
              * `lastHeartbeatTime` (`pulumi.Input[str]`) - Last time we got an update on a given condition.
              ...
              * `type` (`pulumi.Input[str]`) - Type of node condition.

            * `config` (`pulumi.Input[dict]`) - Status of the config assigned to the node via the dynamic Kubelet config feature.
              * `active` (`pulumi.Input[dict]`) - Active reports the checkpointed config the node is actively using. Active will represent either the current version of the Assigned config, or the current LastKnownGood config, depending on whether attempting to use the Assigned config results in an error.
              * `assigned` (`pulumi.Input[dict]`) - Assigned reports the checkpointed config the node will try to use. When Node.Spec.ConfigSource is updated, the node checkpoints the associated config payload to local disk, along with a record indicating intended config. The node refers to this record to choose its config checkpoint, and reports this record in Assigned. Assigned only updates in the status after the record has been checkpointed to disk. When the Kubelet is restarted, it tries to make the Assigned config the Active config by loading and validating the checkpointed payload identified by Assigned.
              * `error` (`pulumi.Input[str]`) - Error describes any problems reconciling the Spec.ConfigSource to the Active config. Errors may occur, for example, attempting to checkpoint Spec.ConfigSource to the local Assigned record, attempting to checkpoint the payload associated with Spec.ConfigSource, attempting to load or validate the Assigned config, etc. Errors may occur at different points while syncing config. Earlier errors (e.g. download or checkpointing errors) will not result in a rollback to LastKnownGood, and may resolve across Kubelet retries. Later errors (e.g. loading or validating a checkpointed config) will result in a rollback to LastKnownGood. In the latter case, it is usually possible to resolve the error by fixing the config assigned in Spec.ConfigSource. You can find additional information for debugging by searching the error message in the Kubelet log. Error is a human-readable description of the error state; machines can check whether or not Error is empty, but should not rely on the stability of the Error text across Kubelet versions.
              * `last_known_good` (`pulumi.Input[dict]`) - LastKnownGood reports the checkpointed config the node will fall back to when it encounters an error attempting to use the Assigned config. The Assigned config becomes the LastKnownGood config when the node determines that the Assigned config is stable and correct. This is currently implemented as a 10-minute soak period starting when the local record of Assigned config is updated. If the Assigned config is Active at the end of this period, it becomes the LastKnownGood. Note that if Spec.ConfigSource is reset to nil (use local defaults), the LastKnownGood is also immediately reset to nil, because the local default config is always assumed good. You should not make assumptions about the node's method of determining config stability and correctness, as this may change or become configurable in the future.

            * `daemon_endpoints` (`pulumi.Input[dict]`) - Endpoints of daemons running on the Node.
              * `kubelet_endpoint` (`pulumi.Input[dict]`) - Endpoint on which Kubelet is listening.
                * `port` (`pulumi.Input[float]`) - Port number of the given endpoint.

            * `images` (`pulumi.Input[list]`) - List of container images on this node
              * `names` (`pulumi.Input[list]`) - Names by which this image is known. e.g. ["k8s.gcr.io/hyperkube:v1.0.7", "dockerhub.io/google_containers/hyperkube:v1.0.7"]
              * `sizeBytes` (`pulumi.Input[float]`) - The size of the image in bytes.

            * `node_info` (`pulumi.Input[dict]`) - Set of ids/uuids to uniquely identify the node. More info: https://kubernetes.io/docs/concepts/nodes/node/#info
              * `architecture` (`pulumi.Input[str]`) - The Architecture reported by the node
              * `boot_id` (`pulumi.Input[str]`) - Boot ID reported by the node.
              * `container_runtime_version` (`pulumi.Input[str]`) - ContainerRuntime Version reported by the node through runtime remote API (e.g. docker://1.5.0).
              * `kernel_version` (`pulumi.Input[str]`) - Kernel Version reported by the node from 'uname -r' (e.g. 3.16.0-0.bpo.4-amd64).
              * `kube_proxy_version` (`pulumi.Input[str]`) - KubeProxy Version reported by the node.
              * `kubelet_version` (`pulumi.Input[str]`) - Kubelet Version reported by the node.
              * `machine_id` (`pulumi.Input[str]`) - MachineID reported by the node. For unique machine identification in the cluster this field is preferred. Learn more from man(5) machine-id: http://man7.org/linux/man-pages/man5/machine-id.5.html
              * `operating_system` (`pulumi.Input[str]`) - The Operating System reported by the node
              * `os_image` (`pulumi.Input[str]`) - OS Image reported by the node from /etc/os-release (e.g. Debian GNU/Linux 7 (wheezy)).
              * `system_uuid` (`pulumi.Input[str]`) - SystemUUID reported by the node. For unique machine identification MachineID is preferred. This field is specific to Red Hat hosts https://access.redhat.com/documentation/en-US/Red_Hat_Subscription_Management/1/html/RHSM/getting-system-uuid.html

            * `phase` (`pulumi.Input[str]`) - NodePhase is the recently observed lifecycle phase of the node. More info: https://kubernetes.io/docs/concepts/nodes/node/#phase The field is never populated, and now is deprecated.
            * `volumes_attached` (`pulumi.Input[list]`) - List of volumes that are attached to the node.
              * `devicePath` (`pulumi.Input[str]`) - DevicePath represents the device path where the volume should be available
              * `name` (`pulumi.Input[str]`) - Name of the attached volume

            * `volumes_in_use` (`pulumi.Input[list]`) - List of attachable volumes in use (mounted) by the node.
