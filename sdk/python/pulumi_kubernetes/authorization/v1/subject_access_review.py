...
class SubjectAccessReview(pulumi.CustomResource):
    ...
    Spec holds information about the request being evaluated
      ...
      * `groups` (`list`) - Groups is the groups you're testing for.
      * `non_resource_attributes` (`dict`) - NonResourceAttributes describes information for a non-resource access request
        * `path` (`str`) - Path is the URL path of the request
        * `verb` (`str`) - Verb is the standard HTTP verb

      * `resource_attributes` (`dict`) - ResourceAuthorizationAttributes describes information for a resource access request
    ...
    def __init__(__self__, resource_name, opts=None, api_version=None, kind=None, metadata=None, spec=None, __props__=None, __name__=None, __opts__=None):
        ...
        The **spec** object supports the following:
          ...
          * `extra` (`pulumi.Input[dict]`) - Extra corresponds to the user.Info.GetExtra() method from the authenticator.  Since that is input to the authorizer it needs a reflection here.
          * `groups` (`pulumi.Input[list]`) - Groups is the groups you're testing for.
          * `non_resource_attributes` (`pulumi.Input[dict]`) - NonResourceAttributes describes information for a non-resource access request
            * `path` (`pulumi.Input[str]`) - Path is the URL path of the request
            * `verb` (`pulumi.Input[str]`) - Verb is the standard HTTP verb

          * `resource_attributes` (`pulumi.Input[dict]`) - ResourceAuthorizationAttributes describes information for a resource access request
            * `group` (`pulumi.Input[str]`) - Group is the API Group of the Resource.  "*" means all.
            * `name` (`pulumi.Input[str]`) - Name is the name of the resource being requested for a "get" or deleted for a "delete". "" (empty) means all.
            * `namespace` (`pulumi.Input[str]`) - Namespace is the namespace of the action being requested.  Currently, there is no distinction between no namespace and all namespaces "" (empty) is defaulted for LocalSubjectAccessReviews "" (empty) is empty for cluster-scoped resources "" (empty) means "all" for namespace scoped resources from a SubjectAccessReview or SelfSubjectAccessReview
            * `resource` (`pulumi.Input[str]`) - Resource is one of the existing resource types.  "*" means all.
            * `subresource` (`pulumi.Input[str]`) - Subresource is one of the existing resource types.  "" means none.
            * `verb` (`pulumi.Input[str]`) - Verb is a kubernetes resource API verb, like: get, list, watch, create, update, delete, proxy.  "*" means all.
            * `version` (`pulumi.Input[str]`) - Version is the API Version of the Resource.  "*" means all.

          * `uid` (`pulumi.Input[str]`) - UID information about the requesting user.
          * `user` (`pulumi.Input[str]`) - User is the user you're testing for. If you specify "User" but not "Groups", then is it interpreted as "What if User were not a member of any groups
        """
