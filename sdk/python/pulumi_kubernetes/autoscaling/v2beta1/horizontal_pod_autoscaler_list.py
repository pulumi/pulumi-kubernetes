        ...
        The **items** object supports the following:
          ...
          * `status` (`pulumi.Input[dict]`) - status is the current information about the autoscaler.
            ...
            * `current_metrics` (`pulumi.Input[list]`) - currentMetrics is the last read state of the metrics used by this autoscaler.
              * `external` (`pulumi.Input[dict]`) - external refers to a global metric that is not associated with any Kubernetes object. It allows autoscaling based on information coming from components running outside of cluster (for example length of queue in cloud messaging service, or QPS from loadbalancer running outside of cluster).
                * `currentAverageValue` (`pulumi.Input[str]`) - currentAverageValue is the current value of metric averaged over autoscaled pods.
                * `currentValue` (`pulumi.Input[str]`) - currentValue is the current value of the metric (as a quantity)
                * `metricName` (`pulumi.Input[str]`) - metricName is the name of a metric used for autoscaling in metric system.
                * `metricSelector` (`pulumi.Input[dict]`) - metricSelector is used to identify a specific time series within a given metric.
              ...
              * `object` (`pulumi.Input[dict]`) - object refers to a metric describing a single kubernetes object (for example, hits-per-second on an Ingress object).
                ...
                * `currentValue` (`pulumi.Input[str]`) - currentValue is the current value of the metric (as a quantity).
                * `metricName` (`pulumi.Input[str]`) - metricName is the name of the metric in question.
                * `selector` (`pulumi.Input[dict]`) - selector is the string-encoded form of a standard kubernetes label selector for the given metric When set in the ObjectMetricSource, it is passed as an additional parameter to the metrics server for more specific metrics scoping. When unset, just the metricName will be used to gather metrics.
                * `target` (`pulumi.Input[dict]`) - target is the described Kubernetes object.

              * `pods` (`pulumi.Input[dict]`) - pods refers to a metric describing each pod in the current scale target (for example, transactions-processed-per-second).  The values will be averaged together before being compared to the target value.
                * `currentAverageValue` (`pulumi.Input[str]`) - currentAverageValue is the current value of the average of the metric across all relevant pods (as a quantity)
                * `metricName` (`pulumi.Input[str]`) - metricName is the name of the metric in question
                * `selector` (`pulumi.Input[dict]`) - selector is the string-encoded form of a standard kubernetes label selector for the given metric When set in the PodsMetricSource, it is passed as an additional parameter to the metrics server for more specific metrics scoping. When unset, just the metricName will be used to gather metrics.

              * `resource` (`pulumi.Input[dict]`) - resource refers to a resource metric (such as those specified in requests and limits) known to Kubernetes describing each pod in the current scale target (e.g. CPU or memory). Such metrics are built in to Kubernetes, and have special scaling options on top of those available to normal per-pod metrics using the "pods" source.
                * `currentAverageUtilization` (`pulumi.Input[float]`) - currentAverageUtilization is the current value of the average of the resource metric across all relevant pods, represented as a percentage of the requested value of the resource for the pods.  It will only be present if `targetAverageValue` was set in the corresponding metric specification.
                * `currentAverageValue` (`pulumi.Input[str]`) - currentAverageValue is the current value of the average of the resource metric across all relevant pods, as a raw value (instead of as a percentage of the request), similar to the "pods" metric source type. It will always be set, regardless of the corresponding metric specification.
                * `name` (`pulumi.Input[str]`) - name is the name of the resource in question.
