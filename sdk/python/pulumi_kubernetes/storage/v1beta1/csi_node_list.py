    ...
    items is the list of CSINode
      ...
      * `spec` (`dict`) - spec is the specification of CSINode
        * `drivers` (`list`) - drivers is a list of information of all CSI Drivers existing on a node. If all drivers in the list are uninstalled, this can become empty.
          * `allocatable` (`dict`) - allocatable represents the volume resources of a node that are available for scheduling.
            * `count` (`float`) - Maximum number of unique volumes managed by the CSI driver that can be used on a node. A volume that is both attached and mounted on a node is considered to be used once, not twice. The same rule applies for a unique volume that is shared among multiple pods on the same node. If this field is nil, then the supported number of volumes on this node is unbounded.
        ...
        The **items** object supports the following:
          ...
          * `spec` (`pulumi.Input[dict]`) - spec is the specification of CSINode
            * `drivers` (`pulumi.Input[list]`) - drivers is a list of information of all CSI Drivers existing on a node. If all drivers in the list are uninstalled, this can become empty.
              * `allocatable` (`pulumi.Input[dict]`) - allocatable represents the volume resources of a node that are available for scheduling.
                * `count` (`pulumi.Input[float]`) - Maximum number of unique volumes managed by the CSI driver that can be used on a node. A volume that is both attached and mounted on a node is considered to be used once, not twice. The same rule applies for a unique volume that is shared among multiple pods on the same node. If this field is nil, then the supported number of volumes on this node is unbounded.
