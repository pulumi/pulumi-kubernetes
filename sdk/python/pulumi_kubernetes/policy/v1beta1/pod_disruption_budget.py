...
class PodDisruptionBudget(pulumi.CustomResource):
    ...
    Specification of the desired behavior of the PodDisruptionBudget.
      * `max_unavailable` (`dict`) - An eviction is allowed if at most "maxUnavailable" pods selected by "selector" are unavailable after the eviction, i.e. even in absence of the evicted pod. For example, one can prevent all voluntary evictions by specifying 0. This is a mutually exclusive setting with "minAvailable".
      * `min_available` (`dict`) - An eviction is allowed if at least "minAvailable" pods selected by "selector" will still be available after the eviction, i.e. even in the absence of the evicted pod.  So for example you can prevent all voluntary evictions by specifying "100%".
      * `selector` (`dict`) - Label query over pods whose evictions are managed by the disruption budget.
    ...
    Most recently observed status of the PodDisruptionBudget.
      * `current_healthy` (`float`) - current number of healthy pods
      * `desired_healthy` (`float`) - minimum desired number of healthy pods
      * `disrupted_pods` (`dict`) - DisruptedPods contains information about pods whose eviction was processed by the API server eviction subresource handler but has not yet been observed by the PodDisruptionBudget controller. A pod will be in this map from the time when the API server processed the eviction request to the time when the pod is seen by PDB controller as having been marked for deletion (or after a timeout). The key in the map is the name of the pod and the value is the time when the API server processed the eviction request. If the deletion didn't occur and a pod is still there it will be removed from the list automatically by PodDisruptionBudget controller after some time. If everything goes smooth this map should be empty for the most of the time. Large number of entries in the map may indicate problems with pod deletions.
      * `disruptions_allowed` (`float`) - Number of pod disruptions that are currently allowed.
      * `expected_pods` (`float`) - total number of pods counted by this disruption budget
      * `observed_generation` (`float`) - Most recent generation observed when updating this PDB status. DisruptionsAllowed and other status information is valid only if observedGeneration equals to PDB's object generation.
    """
