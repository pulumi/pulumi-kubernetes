...
class DaemonSetList(pulumi.CustomResource):
    ...
    A list of daemon sets.
      ...
      * `spec` (`dict`) - The desired behavior of this daemon set. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
        * `min_ready_seconds` (`float`) - The minimum number of seconds for which a newly created DaemonSet pod should be ready without any of its container crashing, for it to be considered available. Defaults to 0 (pod will be considered available as soon as it is ready).
        * `revision_history_limit` (`float`) - The number of old history to retain to allow rollback. This is a pointer to distinguish between explicit zero and not specified. Defaults to 10.
        * `selector` (`dict`) - A label query over pods that are managed by the daemon set. Must match in order to be controlled. If empty, defaulted to labels on Pod template. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors
        ...
        * `template_generation` (`float`) - DEPRECATED. A sequence number representing a specific generation of the template. Populated by the system. It can be set only during the creation.
        * `update_strategy` (`dict`) - An update strategy to replace existing DaemonSet pods with new pods.
          * `rolling_update` (`dict`) - Rolling update config params. Present only if type = "RollingUpdate".
            * `max_unavailable` (`dict`) - The maximum number of DaemonSet pods that can be unavailable during the update. Value can be an absolute number (ex: 5) or a percentage of total number of DaemonSet pods at the start of the update (ex: 10%). Absolute number is calculated from percentage by rounding up. This cannot be 0. Default value is 1. Example: when this is set to 30%, at most 30% of the total number of nodes that should be running the daemon pod (i.e. status.desiredNumberScheduled) can have their pods stopped for an update at any given time. The update starts by stopping at most 30% of those DaemonSet pods and then brings up new DaemonSet pods in their place. Once the new pods are available, it then proceeds onto other DaemonSet pods, thus ensuring that at least 70% of original number of DaemonSet pods are available at all times during the update.

          * `type` (`str`) - Type of daemon set update. Can be "RollingUpdate" or "OnDelete". Default is OnDelete.

      * `status` (`dict`) - The current status of this daemon set. This data may be out of date by some window of time. Populated by the system. Read-only. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
        * `collision_count` (`float`) - Count of hash collisions for the DaemonSet. The DaemonSet controller uses this field as a collision avoidance mechanism when it needs to create the name for the newest ControllerRevision.
        * `conditions` (`list`) - Represents the latest available observations of a DaemonSet's current state.
        ...
        * `current_number_scheduled` (`float`) - The number of nodes that are running at least 1 daemon pod and are supposed to run the daemon pod. More info: https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
        * `desired_number_scheduled` (`float`) - The total number of nodes that should be running the daemon pod (including nodes correctly running the daemon pod). More info: https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
        * `number_available` (`float`) - The number of nodes that should be running the daemon pod and have one or more of the daemon pod running and available (ready for at least spec.minReadySeconds)
        * `number_misscheduled` (`float`) - The number of nodes that are running the daemon pod, but are not supposed to run the daemon pod. More info: https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
        * `number_ready` (`float`) - The number of nodes that should be running the daemon pod and have one or more of the daemon pod running and ready.
        * `number_unavailable` (`float`) - The number of nodes that should be running the daemon pod and have none of the daemon pod running and available (ready for at least spec.minReadySeconds)
        * `observed_generation` (`float`) - The most recent generation observed by the daemon set controller.
        * `updated_number_scheduled` (`float`) - The total number of nodes that are running updated daemon pod
    """
    ...
    def __init__(__self__, resource_name, opts=None, api_version=None, items=None, kind=None, metadata=None, __props__=None, __name__=None, __opts__=None):
        ...
        :param pulumi.Input[list] items: A list of daemon sets.
        ...
        The **items** object supports the following:
          ...
          * `spec` (`pulumi.Input[dict]`) - The desired behavior of this daemon set. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
            * `min_ready_seconds` (`pulumi.Input[float]`) - The minimum number of seconds for which a newly created DaemonSet pod should be ready without any of its container crashing, for it to be considered available. Defaults to 0 (pod will be considered available as soon as it is ready).
            * `revision_history_limit` (`pulumi.Input[float]`) - The number of old history to retain to allow rollback. This is a pointer to distinguish between explicit zero and not specified. Defaults to 10.
            * `selector` (`pulumi.Input[dict]`) - A label query over pods that are managed by the daemon set. Must match in order to be controlled. If empty, defaulted to labels on Pod template. More info: https://kubernetes.io/docs/concepts/overview/working-with-objects/labels/#label-selectors
            ...
            * `template` (`pulumi.Input[dict]`) - An object that describes the pod that will be created. The DaemonSet will create exactly one copy of this pod on every node that matches the template's node selector (or on every node if no node selector is specified). More info: https://kubernetes.io/docs/concepts/workloads/controllers/replicationcontroller#pod-template
              * `metadata` (`pulumi.Input[dict]`) - Standard object's metadata. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#metadata
            ...
            * `template_generation` (`pulumi.Input[float]`) - DEPRECATED. A sequence number representing a specific generation of the template. Populated by the system. It can be set only during the creation.
            * `update_strategy` (`pulumi.Input[dict]`) - An update strategy to replace existing DaemonSet pods with new pods.
              * `rolling_update` (`pulumi.Input[dict]`) - Rolling update config params. Present only if type = "RollingUpdate".
                * `max_unavailable` (`pulumi.Input[dict]`) - The maximum number of DaemonSet pods that can be unavailable during the update. Value can be an absolute number (ex: 5) or a percentage of total number of DaemonSet pods at the start of the update (ex: 10%). Absolute number is calculated from percentage by rounding up. This cannot be 0. Default value is 1. Example: when this is set to 30%, at most 30% of the total number of nodes that should be running the daemon pod (i.e. status.desiredNumberScheduled) can have their pods stopped for an update at any given time. The update starts by stopping at most 30% of those DaemonSet pods and then brings up new DaemonSet pods in their place. Once the new pods are available, it then proceeds onto other DaemonSet pods, thus ensuring that at least 70% of original number of DaemonSet pods are available at all times during the update.

              * `type` (`pulumi.Input[str]`) - Type of daemon set update. Can be "RollingUpdate" or "OnDelete". Default is OnDelete.

          * `status` (`pulumi.Input[dict]`) - The current status of this daemon set. This data may be out of date by some window of time. Populated by the system. Read-only. More info: https://git.k8s.io/community/contributors/devel/sig-architecture/api-conventions.md#spec-and-status
            * `collision_count` (`pulumi.Input[float]`) - Count of hash collisions for the DaemonSet. The DaemonSet controller uses this field as a collision avoidance mechanism when it needs to create the name for the newest ControllerRevision.
            * `conditions` (`pulumi.Input[list]`) - Represents the latest available observations of a DaemonSet's current state.
              ...
              * `type` (`pulumi.Input[str]`) - Type of DaemonSet condition.

            * `current_number_scheduled` (`pulumi.Input[float]`) - The number of nodes that are running at least 1 daemon pod and are supposed to run the daemon pod. More info: https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
            * `desired_number_scheduled` (`pulumi.Input[float]`) - The total number of nodes that should be running the daemon pod (including nodes correctly running the daemon pod). More info: https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
            * `number_available` (`pulumi.Input[float]`) - The number of nodes that should be running the daemon pod and have one or more of the daemon pod running and available (ready for at least spec.minReadySeconds)
            * `number_misscheduled` (`pulumi.Input[float]`) - The number of nodes that are running the daemon pod, but are not supposed to run the daemon pod. More info: https://kubernetes.io/docs/concepts/workloads/controllers/daemonset/
            * `number_ready` (`pulumi.Input[float]`) - The number of nodes that should be running the daemon pod and have one or more of the daemon pod running and ready.
            * `number_unavailable` (`pulumi.Input[float]`) - The number of nodes that should be running the daemon pod and have none of the daemon pod running and available (ready for at least spec.minReadySeconds)
            * `observed_generation` (`pulumi.Input[float]`) - The most recent generation observed by the daemon set controller.
            * `updated_number_scheduled` (`pulumi.Input[float]`) - The total number of nodes that are running updated daemon pod
