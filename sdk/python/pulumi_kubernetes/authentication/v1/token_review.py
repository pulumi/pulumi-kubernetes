...
class TokenReview(pulumi.CustomResource):
    ...
    Spec holds information about the request being evaluated
      * `audiences` (`list`) - Audiences is a list of the identifiers that the resource server presented with the token identifies as. Audience-aware token authenticators will verify that the token was intended for at least one of the audiences in this list. If no audiences are provided, the audience will default to the audience of the Kubernetes apiserver.
      * `token` (`str`) - Token is the opaque bearer token.
    """
    status: pulumi.Output[dict]
    """
    Status is filled in by the server and indicates whether the request can be authenticated.
      * `audiences` (`list`) - Audiences are audience identifiers chosen by the authenticator that are compatible with both the TokenReview and token. An identifier is any identifier in the intersection of the TokenReviewSpec audiences and the token's audiences. A client of the TokenReview API that sets the spec.audiences field should validate that a compatible audience identifier is returned in the status.audiences field to ensure that the TokenReview server is audience aware. If a TokenReview returns an empty status.audience field where status.authenticated is "true", the token is valid against the audience of the Kubernetes API server.
      ...
      * `user` (`dict`) - User is the UserInfo associated with the provided token.
        * `extra` (`dict`) - Any additional information provided by the authenticator.
        * `groups` (`list`) - The names of groups this user is a part of.
    ...
    def __init__(__self__, resource_name, opts=None, api_version=None, kind=None, metadata=None, spec=None, __props__=None, __name__=None, __opts__=None):
        ...
        The **spec** object supports the following:
          ...
          * `token` (`pulumi.Input[str]`) - Token is the opaque bearer token.
        """
