...
func ParseDecodeYamlFiles(ctx *pulumi.Context, args *ConfigGroupArgs, glob bool, clientSet *clients.DynamicClientSet, opts ...pulumi.ResourceOption,
	...
	return ParseYamlObjects(ctx, objs, args.ResourcePrefix, opts...)
}
...
func ParseYamlObjects(
	...
	for _, obj := range objs {
		res, err := parseYamlObject(ctx, obj, resourcePrefix, opts...)
		if err != nil {
	...
	for _, r := range intermediates {
		resources[r.Name] = r.Resource.ID()
	}
	return resources.ToMapOutput(), nil
}
			...
			if items, ok := rawItems.([]interface{}); ok {
				for _, item := range items {
					if obj, ok := item.(map[string]interface{}); ok {
						rs, err := parseYamlObject(ctx, obj, resourcePrefix, opts...)
						if err != nil {
							return nil, err
						}
						for _, r := range rs {
